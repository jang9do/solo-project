...
<div id="wrap">
	...
	<div id="content">
		<form action="${conPath }/animalModify.ani" enctype="multipart/form-data" method="post">
			<table>
				<tr>
				...
				<tr>
					<c:if test="${not empty animal.aBirth }"><fmt:formatDate pattern="YYYY-MM-dd" var="aBirth" value="${animal.aBirth }"/></c:if>
					<th>생일</th>
					<td>
						<c:if test="${empty animal.aBirth }"><input type="date" name="aBirth" />등록안됨</c:if>
						<c:if test="${not empty animal.aBirth }"><input type="date" name="aBirth" value="${aBirth }" /></c:if>
					</td>
				</tr>
				<tr>
					<th>종류</th>
