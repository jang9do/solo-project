...
<div id="wrap">
	...
	<div id="content">
		...
		<p>
			<c:if test="${startPage>10 }"><a href="${conPath }/boardBest.bo?pageNum=${startPage-1 }">[이전]&nbsp;</a></c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="i">
				<c:if test="${i==currentPage }">${i }&nbsp;</c:if>
				<c:if test="${i!=currentPage }"><a href="${conPath }/boardBest.bo?pageNum=${i }">${i }</a>&nbsp;</c:if>
			</c:forEach>
			<c:if test="${endPage!=pageCnt }"><a href="${conPath }/boardBest.bo?pageNum=${endPage+1 }">[다음]</a></c:if>
		</p>
	</div>
