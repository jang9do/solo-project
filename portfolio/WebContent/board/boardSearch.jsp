...
<div id="wrap">
	...
	<div id="content">
		...
		<p>
			<c:if test="${startPage>10 }"><a href="${conPath }/boardSearch.bo?pageNum=${startPage-1 }&rank=${param.rank}&value=${param.value}">[이전]&nbsp;</a></c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="i">
				<c:if test="${i==currentPage }">${i }&nbsp;</c:if>
				<c:if test="${i!=currentPage }"><a href="${conPath }/boardSearch.bo?pageNum=${i }&rank=${param.rank}&value=${param.value}">${i }</a>&nbsp;</c:if>
			</c:forEach>
			<c:if test="${endPage!=pageCnt }"><a href="${conPath }/boardSearch.bo?pageNum=${endPage+1 }&rank=${param.rank}&value=${param.value}">[다음]</a></c:if>
		</p>
